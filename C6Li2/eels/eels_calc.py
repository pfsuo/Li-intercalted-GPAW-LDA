...
def eels_calc(calc, direction='M', eta=25e-3, pbc=True, ecut=100, domega0=0.01, xc='RPA'):
    ...
    #                        integrationmode='tetrahedron_integration',
        ...
        if direction == 'M':
            q_c = np.array([i/50.0, 0.0, 0.0])
        elif direction == 'K':
            q_c = np.array([i/50.0, i/50.0, 0.0])
        elif direction == 'A':
            q_c = np.array([0.0, 0.0, i/51.0])
...
#    responseGS.write('gsresponse.gpw','all')
    ...
    for xc in ['RPA', 'ALDA']:
        for direction in ['A', 'M', 'K']:
            ...
            eels_calc(calc='/opt/gpw_files/C6Li2/gsresponse.gpw', xc=xc, direction=direction)
