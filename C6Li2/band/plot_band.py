...
title = 'Bandstructure of C$_6$Li$_2$'
calc = GPAW('band.gpw')
...
def plotpbanddos(figsize=(10,5),fmt='png'):
    ...
    w[:,:,2] = w_kni[:,:,24:29:4].sum(axis=2) 
    for i in range(24,29,4):
        w[:,:,3] += w_kni[:,:,i+1:i+4].sum(axis=2)
    ...
    pdos[:,0] = pdos_whole[:,:2].sum(axis=1)
    pdos[:,1] = pdos_whole[:,3:5].sum(axis=1)
    pdos[:,2] = pdos_whole[:,2]
    pdos[:,3] = pdos_whole[:,5]
    scale = 60.0
    colors = ['cyan','r']
