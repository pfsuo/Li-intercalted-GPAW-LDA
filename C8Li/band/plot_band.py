...
title = 'Bandstructure of C$_8$Li'
calc = GPAW('band.gpw')
...
def plotpbanddos(figsize=(10,5),fmt='png'):
    ...
    w[:,:,0] = w_kni[:,:,0:29:4].sum(axis=2)
    for i in range(0,29,4):
        w[:,:,1] += w_kni[:,:,i+1:i+4].sum(axis=2)
    w[:,:,2] = w_kni[:,:,32] 
    w[:,:,3] = w_kni[:,:,33:36].sum(axis=2)
    ...
    pdos[:,0] = pdos_whole[:,:8].sum(axis=1)
    pdos[:,1] = pdos_whole[:,9:17].sum(axis=1)
    pdos[:,2] = pdos_whole[:,8]
    pdos[:,3] = pdos_whole[:,17]
    scale = 60.0
    colors = ['cyan','r']
    labels = ['C (p)', 'Li (s)']
    st = []
